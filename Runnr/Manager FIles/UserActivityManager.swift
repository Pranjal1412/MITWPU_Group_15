...
//        guard !isPaused else { return }
        ...
        // Smooth the altitude
        let newSmoothedAltitude = (smoothedAltitude! * 0.7) + (currentAltitude * 0.3)
        
        if let last = elevationLastLocation {
            
            let deltaElevation = newSmoothedAltitude - last.altitude
            
            // Ignore small GPS noise
            if deltaElevation > elevationThreshold {
                totalElevationGain += deltaElevation
            }
        }
        
        smoothedAltitude = newSmoothedAltitude
        
        elevationLastLocation = CLLocation(
    ...
    // Reset elevation when activity fully ends
