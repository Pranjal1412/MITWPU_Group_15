...
class AllActivitiesViewController: UIViewController {
    ...
    func settingLabel() {
        let thinFont = UIFont(
            name: "SFProText-Thin", size: 25
        ) ?? UIFont.systemFont(ofSize: 25, weight: .thin)

        let boldFont = UIFont(
            name: "SFProText-Bold", size: 25
    ...
    @IBAction func buttonCalendarTapped(_ sender: UIButton) {
        ...
        let activityDates = dataSource.getAllActivities().compactMap {
            $0.activity?.activityStartTime
        }
        calendarVC.activityDates = activityDates
...
extension AllActivitiesViewController: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(
        ...
        let shareAction = UIContextualAction(
            ...
            NSLayoutConstraint.activate([
                ...
                indicator.centerYAnchor.constraint(equalTo: loadingAlert.view.centerYAnchor, constant: 16)
            ])
                    ...
                    if let mapURLStr = activity.activity?.mapImageURL,
                       let mapURL = URL(string: mapURLStr) {
                        KingfisherManager.shared.retrieveImage(with: mapURL) { [weak self] result in
                            ...
                            let card = analysisVC.buildShareCardPublic(mapImage: mapImage)
    ...
    private func presentShareSheet(
        ...
        if let popover = shareVC.popoverPresentationController {
            popover.sourceView = sourceTableView.cellForRow(at: indexPath) ?? self.view
            popover.sourceRect = sourceTableView.cellForRow(at: indexPath)?.bounds ?? self.view.bounds
        }
    ...
    func updateScreenElements() {
        ...
        if myActivity.isEmpty {
            ...
            let thinFont = UIFont(
                name: "SFProText-Thin", size: 22
            ) ?? UIFont.systemFont(ofSize: 22, weight: .thin)

            let boldFont = UIFont(
                name: "SFProText-Bold", size: 22
